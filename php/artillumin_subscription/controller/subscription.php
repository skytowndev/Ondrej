...
class ControllerAccountSubscription extends Controller {
	public function index() {
		...
		$subscription_id = $this->customer->getActiveCustomerSubscription();
		
		$data['subscription'] = array();
		$data['products'] = array();
		if ($subscription_id > 0) {
			$result = $this->model_account_subscription->getSubscription($subscription_id);
			if ($result ) {
				$data['subscription'] = array(
					...
					'date_finish'           => date($this->language->get('date_format_short'), strtotime($result['date_finish'])),
			...
			$subscription_id = $this->model_account_subscription->getPendingSubscriptionId();
			if ($subscription_id > 0) {
				$result = $this->model_account_subscription->getSubscription($subscription_id);

				if ($result ) {
					$data['subscription_pending'] = array(
						'subscription_id'		=> $result['subscription_id'],
						'order_id'				=> $result['order_id'],
						'date_added'            => date($this->language->get('date_format_short'), strtotime($result['date_added'])),
						'date_start'          	=> date($this->language->get('date_format_short'), strtotime($result['date_start'])),
			...
			$results = $this->model_catalog_product->getProducts(array('is_subscription_plan' => true));
			...
			foreach ($results as $result) {
				...
				$subscription_plan = $this->model_catalog_product->getProductSubscriptionPlan($result['product_id']);
		...
		$data['checkout_url'] = $this->url->link('extension/quickcheckout/checkout', '', true);
